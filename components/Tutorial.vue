...
<template>
  <div class="relative flex items-top justify-center min-h-screen bg-surface-700 sm:items-center sm:pt-0 font-body">
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
      ...
      <div class="mt-8 bg-surface-500 text-white overflow-hidden shadow sm:rounded-lg p-6">
        <h2 class="text-2xl leading-7 font-semibold mb-2">
          Welcome to your Nuxt Application
        </h2>
        <button class="btn mr-4" type="button" role="button" @click="sheet = !sheet">
          {{ $t('clickHere') }}
        </button>
        <button class="btn-danger mr-4" type="button" role="button" @click="$nuxt.$emit('ShowAccountDeleteDialog')">
          {{ $t('buttons.deleteAccount') }}
        </button>
        <button class="btn mr-4" type="button" role="button" @click="$toast.show('Showing toast')">
          {{ $t('clickHere') }}
        </button>
        <button class="btn" type="button" role="button" @click="$modal.show({ title: 'Important', body: 'Open components/Tutorial.vue and start coding in in pages/index.vue.', primary: { label: 'Okay', action: () => {} }, persistent: true })">
          {{ $t('clickHere') }}
        </button>
    ...
    <bottom-sheet v-model="sheet" overlay click-to-close max-width="100vw" rounded swipe-able>
      <div class="h-screen">
        <h1>Title</h1>
      </div>
    </bottom-sheet>
